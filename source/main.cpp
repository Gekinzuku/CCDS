...
****1.0        12/31/07      GeekyLink          Finished! Submitted to Drunken Coder's "3rd Annual Christmas Coding Contest". Placed 2nd!!!
 *  1.1B       07/09/08      GeekyLink          Recoding huge sections of code for CCDS remake.
 **************************************************************************************************/
...
#include "enemyengine.h"//the enemy engine (non existent atm)
...
bool DisplayHint = false; // Displays the hint
...
u8 iTimer = 0; //used for making sure things have the right amount of time between events
...
bool bFlippers; //if chip has flippers
u8 iRedKey; //red key
...
void PlayGame()
{
	...
	while(cThing!='t')
	{
      ...
      if(iTimer > 59)
      {
		...
		PA_OutputText(1,1,1,"Time Remaining: %d   ", iTime);  
	   PA_OutputText(1,1,4,"Chips: %d  ", iChips);
	   
	   //keys
	   
	   if (Pad.Newpress.L)
	   	cHint = "I'm a fucking pimp, bitch!";
	   else if (Pad.Newpress.R)
	   	cHint = "Suck my dick and I'll give you a dollar.";
	   
	   PA_OutputText(1,1,8,"Red Keys: %d ", iRedKey);
	   PA_OutputText(1,1,9,"Green Keys: %d ", iGreenKey);
	   PA_OutputText(1,1,10,"Yellow Keys: %d ", iYellowKey);
	   PA_OutputText(1,1,11,"Blue Keys: %d ", iBlueKey);
	   
		PA_OutputText(1,1,13,"Moves used: %d     ", iMove);
		if (DisplayHint == true)
			PA_OutputText(1,1,15,"Tip: %s", cHint);
	   
		//Replaced move limit with time limit 
		if(iTime < 1)
		{
			InitRoom(); //restarts the room 
		 	PA_PlaySimpleSound(die);
		}		
     
		//Music();

     	PA_SetSpriteAnim(0,0,iDire + (chip.swimming*4));
...
void Move()
{
  	u16 i = u16(chip.x*0.03125);
	u16 i2 = u16(chip.y*0.03125);
   ...
   if(cTile[i][i2] == 'f') //chip stepped on fire
	...
	{
		PA_PlaySimpleSound(win);//sound of winning	
	...
	for (i=0;i<8;i++)
	{
		for(i2=0;i2<6;i2++)
		{
			...
			else if(cTile[i][i2] == 'i')//sets ice
			{
   			...
   			else if((cTile[i][i2] == 't') || (cTile[i][i2] == 'T') || (cTile[i][i2] == 'p')) //sets tiles
			{
				PA_SetSpriteAnim(0, 60 + 6*i + i2, 0);
				PA_SetSpriteAnim(0, 11 + 6*i + i2, 0); //erases anything above the tile
			}
			else if(cTile[i][i2] == 'u') //sets pop up wall
			{
				PA_SetSpriteAnim(0, 11 + 6*i + i2, 0);
				PA_SetSpriteAnim(0, 60 + 6*i + i2, 21); 
			}
			...
			else if(cTile[i][i2] == '2') //sets ice turn
			{
				PA_SetSpriteAnim(0, 60 + 6*i + i2, 6); 
				PA_SetSpriteAnim(0, 11 + 6*i + i2, 0); //erases anything above the tile
			}
			else if(cTile[i][i2] == '3') //sets ice turn
			{
				PA_SetSpriteAnim(0, 60 + 6*i + i2, 7); 
				PA_SetSpriteAnim(0, 11 + 6*i + i2, 0); //erases anything above the tile
			}
			else if(cTile[i][i2] == '4') //sets ice turn
			{
			...
			else if((cTile[i][i2] == 'G') || (cTile[i][i2] == 'g'))  //sets green blocks
			{
				PA_SetSpriteAnim(0, 11 + 6*i + i2, 13); 
				PA_SetSpriteAnim(0, 60 + 6*i + i2, 0); 
			}
			else if(cTile[i][i2] == 'h') //sets hint
			{ 
				PA_SetSpriteAnim(0, 11 + 6*i + i2, 0);
				PA_SetSpriteAnim(0, 60 + 6*i + i2, 22);
			}
...
u8 CheckSpot() //checks to see what is in the spot that chip is stepping in
{
	...
	{
		PA_PlaySimpleSound(oof);//sound of chip running into wall
		chip.OnTheMove = false;
	 	return 1;
	}
	else if(cTile[i][i2] == 'c')//if you walk over a chip
	{
		PA_PlaySimpleSound(pickupchip);//sound of a chip
	...
	else if(cTile[i][i2] == 's')//if you walk over a socket
	{
	   if(iChips == 0)//if all the chips are gone
		{   
			...
			PA_PlaySimpleSound(door); //sound of door
			return 0;
		}		
		else //if there are chips left
		{
		   PA_PlaySimpleSound(oof);//plays sound and doesn't let chip in
		   chip.OnTheMove = false;
		   return 1;
		}		   
	}
	...
	else if((cTile[i][i2] == 'T') || (cTile[i][i2] == 'G'))//Fake tile that turns into a wall
	{
	   cRoom[iRoomX][iRoomY] = 'w';
	   PA_PlaySimpleSound(oof);//plays sound
	   chip.OnTheMove = false;
	   return 1;
	}
	else if((cTile[i][i2] == 'W') || (cTile[i][i2] == 'g'))//Fake wall that turns into a tile
	{
	   cRoom[iRoomX][iRoomY] = 't';
	   return 0;
	}
	else if(cTile[i][i2] == 'I')//ice skates
	{
	   cRoom[iRoomX][iRoomY] = 't';
	   bSkates = true;
	   PA_PlaySimpleSound(item); //sound of picking up item
	   return 0;
	}
	else if(cTile[i][i2] == 'B')//button
	{
	   ...
	   PA_PlaySimpleSound(button); //sound of hitting button
	   return 0;
	}
	else if(cTile[i][i2] == '?')//switch wall defualt up
	{
	   if(bSwitch == false){PA_PlaySimpleSound(oof); chip.OnTheMove = false; return 1;}
	   else return 0;
	}
	else if(cTile[i][i2] == '/')//switch wall defualt down
	{
	   if(bSwitch == false)return 0; //tells the computer the button was pressed
	   else{PA_PlaySimpleSound(oof); chip.OnTheMove = false; return 1;}
	}
	...
	else if(cTile[i][i2] == '%')//red key door
	{
		if(iRedKey > 0){
	   	...
	   	PA_PlaySimpleSound(oof); //sound of walking into wall
	   	chip.OnTheMove = false;
			return 1;}
	}
	...
	else if(cTile[i][i2] == 'h')
	{
		DisplayHint = true;
		PA_PlaySimpleSound(button);
		return 0;
	}	
	...
	{//allows you to push the thing onto water or tiles
	   if((iDire==2) && ((cRoom[iRoomX+1][iRoomY] == 't') || (cRoom[iRoomX+1][iRoomY] == 'a'))){
			cRoom[iRoomX][iRoomY] = 't';
			if(cRoom[iRoomX+1][iRoomY] == 'a'){PA_PlaySimpleSound(water); cRoom[iRoomX+1][iRoomY] = 'd';}
			...
			cRoom[iRoomX][iRoomY] = 't';
			if(cRoom[iRoomX][iRoomY-1] == 'a'){PA_PlaySimpleSound(water); cRoom[iRoomX][iRoomY-1] = 'd';}
...
void Slide()//used for sliding on ice
{
   u16 i = u16(chip.x*0.03125);
	...
	else if (cTile[i][i2] == 'a')
	{
		cBusy = 't';   
	}	
	else if(cTile[i][i2] == 'u') //chip slide on pop-up wall
	{
		cBusy = 't';
	   cRoom[iRoomX][iRoomY] = 'w';
	}	
	else if(cTile[i][i2] == 'T') //chip slide on pop-up wall
	{
	   cRoom[iRoomX][iRoomY] = 'w';
	}
	...
	else if((cTile[i][i2] == 'W') || (cTile[i][i2] == 'g'))  //fake wall
	{
  	   cRoom[iRoomX][iRoomY] = 't';
	   cBusy = 't';
	}	
	...
	else if(cTile[i][i2] == 'A') //pick up flippers
	{
	   PA_PlaySimpleSound(item); //sound of picking up item
  	   cRoom[iRoomX][iRoomY] = 't';
  	   bFlippers = true;
	...
	{	   
	   ...
	   if(iDire==0){
		...
		if(iDire==0){
			...
			{
			  if (cRoom[iRoomX][iRoomY+2] == 't')
			   {
			   	cRoom[iRoomX][iRoomY+2] = 'P';
			   	cRoom[iRoomX][iRoomY+1] = 't';
				}   
			   else			   	
					iDire=1;
			}
			else if((cTile[i][i2+1] == 'T') || (cTile[i][i2+1] == 'G')) //if you hit a fake tile or green wall
			{
				iDire=1;
				cRoom[iRoomX][iRoomY+1] = 'w'; // Put wall there
			}
			...
			else if(cTile[i][i2-1] == 'P')//if you hit a push block
			{
			  if (cRoom[iRoomX][iRoomY-2] == 't')
			   {
			   	cRoom[iRoomX][iRoomY-2] = 'P';
			...
			{
			   if (cRoom[iRoomX+2][iRoomY] == 't')
			   {
			   	cRoom[iRoomX+2][iRoomY] = 'P';
			   	cRoom[iRoomX+1][iRoomY] = 't';
				}   
			   else			   	
					iDire=3;
			}
			else if((cTile[i+1][i2] == 'T') || (cTile[i+1][i2] == 'G')) //if you hit a fake tile or green wall
			{
				iDire=3;
				cRoom[iRoomX+1][iRoomY] = 'w'; // Put wall there
			}
...
void CreateAll() //creats all the sprites ahead of time
{
	...
	PA_LoadSpritePal(0, 4,(void*)button_Pal); //number palette
	PA_LoadSpritePal(0, 5,(void*)bigbuttons_Pal); //number palette
	//top screen
	...
	PA_LoadSpritePal(1, 4,(void*)options_Pal); //number palette
	
	//buttons for sorting through the level select screen
	PA_CreateSprite(0, 122,(void*)button_Sprite, OBJ_SIZE_32X32,1, 4, -32, -32);
	PA_CreateSprite(0, 123,(void*)bigbuttons_Sprite, OBJ_SIZE_64X32,1, 5, -64, -32);
	PA_CreateSprite(0, 121,(void*)button_Sprite, OBJ_SIZE_32X32,1, 4, -32, -32);
	PA_CreateSprite(1, 124,(void*)options_Sprite, OBJ_SIZE_16X16,1, 4, -16, -16);
